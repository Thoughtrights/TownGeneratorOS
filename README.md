...
| `moat` | `0` / `1` | `1` digs a water-filled moat ringing the wall (requires `walls=1`), with a drawbridge at every land gate. Outskirt sprawl beyond the wall is suppressed — nothing builds in or over the moat. A river or the sea merges into it, making a river-fed or sea-fed moat. Default `0`. |
...
- **`moat=1`** ray-casts a smooth ring of water off the whole fortification — the wall circuit and the citadel's own wall — with a dry berm so no wall or tower ever stands in the water. Roads stop at the banks and cross only on the **drawbridges** at the gates.
