...
| `moat` | `0` / `1` | `1` digs a water-filled moat ringing the wall (requires `walls=1`). Roads carry straight across it, their edges switching to the wall colour for the wet stretch — reading as built causeways at the gates. Outskirt sprawl beyond the wall is suppressed — nothing builds in or over the moat. A river or the sea merges into it, making a river-fed or sea-fed moat. Default `0`. |
...
- **`moat=1`** ray-casts a smooth ring of water off the whole fortification — the wall circuit and the citadel's own wall — with a dry berm so no wall or tower ever stands in the water. Roads carry straight over the moat as **causeways**: for the wet stretch the road's edging takes the wall colour, so each gate's crossing reads as built masonry rather than a floating deck.
