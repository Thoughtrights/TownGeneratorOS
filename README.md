...
| `terrain` | `0`-`4` | Surrounding terrain. `0` (default) none; `1` forest — fused organic groves with bumpy edges and lone trees between; `2` mountains — top-down topographic contour lines traced from a summed elevation field, so neighbouring rises merge additively into one ring system (substantial massifs plus smaller outcroppings); `3` swamp (grass tufts and damp pools); `4` cavern — the whole city sits inside a giant cave, rock-dark beyond a jagged rim. All terrain avoids water, roads, farms, and buildings. |
